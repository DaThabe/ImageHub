...
internal sealed partial class PixivMetadataExtractor(ILogger<PixivMetadataExtractor> logger) : IMetadataExtractor
{
    ...
    public async Task<Metadata> GetAsync(IPage page, Source source, CancellationToken cancellationToken = default)
    {
        ...
        if (await btn.CountAsync() > 0)
        {
            ...
            logger.LogDebug("已点击展开按钮, 按钮内容:{content}", await btn.InnerTextAsync());
