...
internal sealed partial class TelegramPublisher(
    ...
    ) : IDomainEventHandler<JobResourcesReadyDomainEvent>
{
    public async Task HandleAsync(JobResourcesReadyDomainEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                logger.LogInformation("正在发送第 {cur}/{max} 组图像...", groupIndex, groups.Count);
    ...
    private async Task<InputMediaPhoto?> ProcessSingleImage(string filePath, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            logger.LogInformation("正在压缩图像: {FileName}", fileName);
        ...
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "❌ 图像处理失败: {FileName}", fileName);
            return null;
        }
    }
    // 计算每组图像数量
    ...
    private static string ToHtml(JobResourcesReadyDomainEvent @event)
    {
        ...
        {
            sb.Append($"""
                {Environment.NewLine}标签: {string.Join("", tags.Select(x => $"#{x} "))}
                """);
        }
